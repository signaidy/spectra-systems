<script lang="ts">
  ...
  const handleCheckout = (flightId, passengers, category) => {
    ...
    if (flight.scale != null) {
      ...
      goto(`/bundleHotel?flight_id=${flightId}&guests=${passengers}&category=${category}&check-in=${flight.arrivalDate}&check-out=${flight.returnFlight.departureDate}&city=${flight.destinationCityName}`);
    }
  };
</script>
