<script>
    ...
    onMount(async () => {
      fetch('http://localhost:8080/purchaselogs')
        .then((response) => response.json())
        .then((userpurchases) => {
          purchaseslogs = userpurchases;
          if (purchaseslogs.length > 0) {
          billData.customer.name = purchaseslogs[0].user_name
          billData.items.total = purchaseslogs[0].price
          billData.items.unitprice = purchaseslogs[0].price
          billData.subtotal = purchaseslogs[0].price
          billData.total = purchaseslogs[0].price
          billData.invoiceNumber = purchaseslogs[0].purchase_number
          billData.date = purchaseslogs[0].purchase_date
          billData.ticket.origin = purchaseslogs[0].origin
          billData.ticket.destination = purchaseslogs[0].destination
          billData.ticket.departure = purchaseslogs[0].departure_date
          billData.ticket.arrival = purchaseslogs[0].arrival_date
          billData.ticket.paymenth_method = purchaseslogs[0].paymenth_method
          }
        });
    });
  ...
  <div>
    {#if purchaseslogs.length > 0}
      {#each purchaseslogs as { purchase_number, ticket, type, origin, destination, purchase_date, price, paymenth_method, arrival_date, departure_date, user_name }}
        <div class="p-10">
          <div
          ...
          >
            ...
            <p class="ml-10 uppercase font-bold text-black-500">User name: {user_name}</p>
